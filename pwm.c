...
void Motor_Forward(void)
{
	...
	// 앞쪽, 뒤쪽 좌측 모터
	//TIM3->CCR2 = 100;
	PWM_SetDutyCycle(TIM_CHANNEL_2, 200);  // PWM 채널 2
...
{
	// 앞쪽, 뒤쪽 우측 모터
	PWM_SetDutyCycle(TIM_CHANNEL_1, 100);  // PWM 채널 1
	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_4, GPIO_PIN_RESET);  // 방향 제어 핀
	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_5, GPIO_PIN_RESET);

	// 앞쪽, 뒤쪽 좌측 모터
	PWM_SetDutyCycle(TIM_CHANNEL_2, 100);  // PWM 채널 2
	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_13, GPIO_PIN_SET);  // 방향 제어 핀
	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_14, GPIO_PIN_RESET);
}
...
// 앞쪽, 뒤쪽 우측 모터
	...
	// 앞쪽, 뒤쪽 좌측 모터
	PWM_SetDutyCycle(TIM_CHANNEL_2, 1000);  // PWM 채널 2
	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_13, GPIO_PIN_RESET);  // 방향 제어 핀
	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_14, GPIO_PIN_RESET);
}
...
void Auto_Drive(void)
{
	if(mode == 2)
	{
		if(distance2 >= 40)		// 전방에 장애물이 없다면
		{
			...
			else if(distance3 < 25)		// distance3 < 20
			{
				Motor_Left_slow();
			}
		}
		...
		else if(distance2 < 25)		// 전방에 장애물이 있다면
		{
			...
			else if((distance1 > distance3) || (distance3 < 25))
			{
				Motor_Left_slow();
			}
		}
